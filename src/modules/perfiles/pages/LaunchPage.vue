<template>
    <DashboardLayout>
      <div class="p-8 bg-gray-50">
        ...
        <!-- Formulario -->
        <form @submit.prevent="lanzarProceso" class="bg-white p-6 rounded-lg shadow-md">
            ...
            <div v-if="mostrarConfigAvanzada" class="space-y-3 mb-6">
              ...
              <div class="flex items-center justify-between space-x-4">
                <label class="text-gray-700">
                  {{ tipoAmbiente === 'controlado' && solicitarRegistro ? 'Solicitar registro (El evaluado debe ingresar con su identificación)' : 'Solicitar registro (El evaluado debe registrar una cuenta)' }}
                </label>
                <input type="checkbox" class="toggle" v-model="solicitarRegistro" />
              </div>
              ...
              <div class="space-y-3">
                ...
                <div v-if="tipoAmbiente === 'controlado' && !solicitarRegistro" class="flex items-center justify-between space-x-4">
                  <label class="text-gray-700">Mostrar detalles a la empresa</label>
                  <input type="checkbox" class="toggle" v-model="mostrarDetallesEmpresa" />
                </div>
              </div>
            </div>
          </transition>
